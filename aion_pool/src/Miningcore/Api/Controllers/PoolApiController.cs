...
namespace Miningcore.Api.Controllers
{
    ...
    public class PoolApiController : ControllerBase
    {
        public PoolApiController(IComponentContext ctx)
        {
            ...
            invalidShareRepo = ctx.Resolve<IInvalidShareRepository>();
        ...
        private readonly IInvalidShareRepository invalidShareRepo;
            ...
            string poolId, [FromQuery] int page, [FromQuery] int pageSize, [FromQuery] BlockStatus[] state)
        {
            ...
            var blocks = (await cf.Run(con => blocksRepo.PageDistinctBlocksAsync(con, pool.Id, blockStates, page, pageSize)))
        ...
        public async Task<PagedResponse<Responses.Payment>> PagePoolPaymentsAsync(
            string poolId, [FromQuery] int page, [FromQuery] int pageSize)
        {
            var pool = GetPool(poolId);

            var result = (await cf.Run(con => paymentsRepo.PagePaymentsAsync(
            ...
            return new PagedResponse<Responses.Payment>
            {
                Total = result.Total,
                Results = payments
            };
        ...
        [HttpGet("{poolId}/stats/miners")]
        public async Task<PoolValueStatResponse[]> GetMiningPoolMinersStats(string poolId, long start, long end)
        {
            var (startDate, endDate) = parseStartEndRequestParams(start, end);
            PoolValueStat[] stats = await cf.Run(con => statsRepo.GetPoolConnectedMiners(con, poolId, startDate, endDate));
            return stats
            .Select(a => { var b = new PoolValueStatResponse(); b.Timestamp = ((DateTimeOffset)a.Timestamp).ToUnixTimeSeconds(); b.Value = a.Value; return b; })
            .ToArray();
        }


        [HttpGet("{poolId}/stats/hashrate")]
        public async Task<PoolValueStatResponse[]> GetMiningPoolHashrateStats(string poolId, long start, long end)
        {
            var (startDate, endDate) = parseStartEndRequestParams(start, end);
            PoolValueStat[] stats = await cf.Run(con => statsRepo.GetPoolHashrate(con, poolId, startDate, endDate));
            return stats
            .Select(a => { var b = new PoolValueStatResponse(); b.Timestamp = ((DateTimeOffset)a.Timestamp).ToUnixTimeSeconds(); b.Value = a.Value; return b; })
            .ToArray();
        }

        [HttpGet("{poolId}/stats/percentage")]
        public async Task<PoolValueStatResponse[]> GetMiningPoolPercentageStats(string poolId, long start, long end)
        {
            var (startDate, endDate) = parseStartEndRequestParams(start, end);
            PoolValueStat[] stats = await cf.Run(con => statsRepo.GetPoolNetworkPercentage(con, poolId, startDate, endDate));
            return stats
            .Select(a => { var b = new PoolValueStatResponse(); b.Timestamp = ((DateTimeOffset)a.Timestamp).ToUnixTimeSeconds(); b.Value = a.Value; return b; })
            .ToArray();
        }

        [HttpGet("{poolId}/stats/payments")]
        public async Task<PoolValueStatResponse[]> GetMiningPoolPaymentStats(string poolId, long start, long end)
        {
            var (startDate, endDate) = parseStartEndRequestParams(start, end);
            PoolValueStat[] stats = await cf.Run(con => statsRepo.GetPoolPaid(con, poolId, startDate, endDate));
            return stats
            .Select(a => { var b = new PoolValueStatResponse(); b.Timestamp = ((DateTimeOffset)a.Timestamp).ToUnixTimeSeconds(); b.Value = a.Value; return b; })
            .ToArray();
        }

        [HttpGet("{poolId}/networkDifficulty")]
        public async Task<PoolValueStatResponse[]> GetNetworkDifficulty(string poolId, long start, long end)
        {
            var (startDate, endDate) = parseStartEndRequestParams(start, end);
            PoolValueStat[] stats = await cf.Run(con => statsRepo.GetNetworkDifficulty(con, poolId, startDate, endDate));
            return stats
            .Select(a => { var b = new PoolValueStatResponse(); b.Timestamp = ((DateTimeOffset)a.Timestamp).ToUnixTimeSeconds(); b.Value = a.Value; return b; })
            .ToArray();
        }
        ...
        public async Task<PagedResponse<Responses.Payment>> PageMinerPaymentsAsync(
        ...
        private (DateTime, DateTime) parseStartEndRequestParams(long start, long end)
        {
            // start and end are unix timestamps
            var startDate = fromUnixTimeStamp(start, DateTime.MinValue);
            var endDate = fromUnixTimeStamp(end, DateTime.UtcNow);

            return (startDate, endDate);
        }

        private DateTime fromUnixTimeStamp(long timestamp, DateTime defaultValue)
        {
            if (timestamp == 0)
                return defaultValue;
